...
module icache_sram #(
  ...
  wire [DATA_WIDTH-1:0] data_out = packed_out[DATA_HI:DATA_LO];
  wire [TAG_BITS-1:0] tag_out = packed_out[TAG_HI:TAG_LO];
  wire tag_match = valid[idx] && (tag_out == tag);
  always @(*) begin
    hit   = tag_match;
    rdata = tag_match ? data_out : {DATA_WIDTH{1'b0}};
  end
  always @(posedge clk) begin
    if (!resetn || flush) valid <= {NUM_LINES{1'b0}};
    else if (we) valid[idx] <= 1'b1;
  end
endmodule
